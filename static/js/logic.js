...
.then(function(plateData){
    ...
    L.geoJson(plateData, {
        // add stying to make the lines visible
        color: "yellow",
        weight: 1
...
.then(
    function(earthquakeData){
        ...
        // plot circles where the radius is dependent on the magnitude and color on the depth
...
legend.onAdd = function() {
    ...
    // loop through the intervals and the colors and generate a label with colored square for each interval
    for(var i = 0; i < intervals.length; i++)
    {
        ...
        div.innerHTML += "<i style='background: "
            ...
            + "'></i> "
            + intervals[i]
            + (intervals[i + 1] ? "km &dash km;" + intervals[i + 1] + "km<br>" : "+");
    }
